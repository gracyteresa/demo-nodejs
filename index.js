...
mongoose.connect("mongodb+srv://neelima:neelima@neelima.yhq62.mongodb.net/bus-booking")
const app = express();
...
app.get('/booked-buses', async (req, res) => {
	const loginUser = req.cookies.loginUserId;
	const userData = await User.findById(loginUser)?.populate('bookedBuses')?.lean();
...
app.post("/login", async (req, res) => {
	try {
		...
		const userData = await User.findOne({ mail: req.body.mail, password: req.body.password, isAdmin: isAdmin })?.lean();
		if (userData) {
...
app.post("/add-bus", async (req, res) => {
	try {
		...
		await Bus.create({
			...
			busImage: data?.busImage?.length ? data?.busImage : 'https://t3.ftcdn.net/jpg/05/71/69/10/360_F_571691018_GxAIRdpQ1wk38db2lYkWQEhxqalnBsL3.jpg',
			driverDetails: {
